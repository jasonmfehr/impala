...
string to_string(const ChildSpanType& val) {
  return CHILD_SPAN_NAMES[static_cast<int>(val)];
}

ostream& operator<<(ostream& out, const ChildSpanType& val) {
  out << to_string(val);
  return out;
}
...
SpanManager::SpanManager(nostd::shared_ptr<trace::Tracer> tracer,
    ClientRequestState* client_request_state) : tracer_(move(tracer)),
