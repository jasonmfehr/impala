...
const string WM_SCHEMA_VER_PROP_NAME_OLD = "schema_version";
const string WM_SCHEMA_VER_PROP_NAME_NEW = "wm_schema_version";

/// Returns the fully qualified table name in format "db.tbl_name".
static string _fullTableName(const string db, const string tbl_name) {
...
static TCatalogServiceRequestHeader _getHeader() {
  TCatalogServiceRequestHeader req_header;
  req_header.__set_client_ip("127.0.0.1");
...
/// Equivalent to: DROP TABLE {{db}}.{{table}} IF NOT EXISTS PURGE.
static Status _dropTable(CatalogServiceIf* svc, const string db, const string tbl_name) {
...
static Status _setupDb(CatalogServiceIf* svc, const string db) {
  TDdlExecResponse resp;
  TDdlExecRequest req;
...
///                STORED AS ICEBERG
static Status _createTable(CatalogServiceIf* svc, const string& table_name,
    const Version& target_version, bool is_system_table) {
  ...
  map<string, string> table_props = {
      make_pair(WM_SCHEMA_VER_PROP_NAME_OLD, VERSION_1_0_0.ToString()),
      make_pair(WM_SCHEMA_VER_PROP_NAME_NEW, target_version.ToString()),
      make_pair("format-version", "2"),
      make_pair("OBJCAPABILITIES", "EXTREAD,EXTWRITE")};
  ...
  if (!FLAGS_query_log_table_props.empty()) {
    for (const auto& prop : strings::Split(FLAGS_query_log_table_props, ",")) {
      ...
      if (prop_parts.size() != 2) {
        return Status(StrCat("property '", prop, "' is not in the expected 'key=value' "
            "format"));
      }
  ...
  // Table columns.
  t_create_tbl.__set_columns(_buildCols(
  ...
  if (is_system_table) {
    ...
    t_partition_identity.__set_field_name(to_lower_copy(
        to_string(TQueryTableColumn::CLUSTER_ID)));
    t_partition_identity.__set_orig_field_name(to_lower_copy(
        to_string(TQueryTableColumn::CLUSTER_ID)));
    t_partition_identity.__set_transform(t_transform_identity);

    t_transform_hour.__set_transform_type(
        TIcebergPartitionTransformType::type::HOUR);
    t_partition_start_hour.__set_field_name(to_lower_copy(
        to_string(TQueryTableColumn::START_TIME_UTC)));
    t_partition_start_hour.__set_orig_field_name(to_lower_copy(
        to_string(TQueryTableColumn::START_TIME_UTC)));
...
static Status _upgradeTable(CatalogServiceIf* svc, const string& table_name,
  ...
  t_add_cols.__set_columns(_buildCols(
      [&current_version, &target_version](const FieldDefinition& f){
...
static Status _getTableSchemaVersion(CatalogServiceIf* svc, const string db,
    const string tbl, Version* table_version) {
  ...
  reset_req.__set_header(_getHeader());
  ...
  // Determine the actual table schema version
  optional<Version> schema_version_old;
  optional<Version> schema_version_new;
  ...
  for (const auto& iter : o_resp.catalog_object.table.metastore_table.parameters) {
    if (iter.first == WM_SCHEMA_VER_PROP_NAME_OLD) {
      schema_version_old = Version();
      if (!ParseVersion(iter.second, &*schema_version_old).ok()) {
        return Status(StrCat("could not parse version string '", iter.second, "' found "
            "on the '", WM_SCHEMA_VER_PROP_NAME_OLD, "' property of table '", db, ".",
            tbl, "'"));
      }
    } else if (iter.first == WM_SCHEMA_VER_PROP_NAME_NEW) {
      schema_version_new = Version();
      if (!ParseVersion(iter.second, &*schema_version_new).ok()) {
        return Status(StrCat("could not parse version string '", iter.second, "' found "
            "on the '", WM_SCHEMA_VER_PROP_NAME_NEW, "' property of table '", db, ".",
            tbl, "'"));
      }

      // The new schema version property takes precendence, thus when it is encountered,
      // there is no need to continue looking for the old schema version.
      break;
    }
  }

  // New schema version property takes precendence over the old property if both exist.
  if (schema_version_new.has_value()) {
    *table_version = move(*schema_version_new);
    return Status::OK();
  } else if (schema_version_old.has_value()) {
    *table_version = move(*schema_version_old);
    return Status::OK();
  }

  return Status(StrCat("Neither property '", WM_SCHEMA_VER_PROP_NAME_OLD, "' nor "
      "property '", WM_SCHEMA_VER_PROP_NAME_NEW, "' exists on table '", db, ".", tbl,
      "'" ));
...
static Status _tableSchemaManagement(CatalogServiceIf* svc, const string table_name,
    const Version target_schema_version, const bool is_system_table) {
  Version parsed_actual_schema_version;

  // Create and/or update the table if needed.
  RETURN_IF_ERROR(_getTableSchemaVersion(svc, WM_DB, table_name,
      &parsed_actual_schema_version));
  ...
  if (parsed_actual_schema_version == target_schema_version) {
    ...
    RETURN_IF_ERROR(_createTable(svc, table_name, target_schema_version,
        is_system_table));
    ...
    LOG(WARNING) << "Target schema version '" << target_schema_version.ToString() <<
        "' of the '" << full_table_name << "' table is lower than the actual schema " <<
    ...
    RETURN_IF_ERROR(_upgradeTable(svc, table_name, parsed_actual_schema_version,
...
Status CatalogServer::InitWorkloadManagement() {
  if (!FLAGS_enable_workload_mgmt) {
    LOG(INFO) << "Skipping workload management initialization since workload management "
        << "is not enabled";
    return Status::OK();
  }

  if (!IsActive()) {
    LOG(INFO) << "Skipping workload management initialization since this catalogd is not "
        << "the active catalogd";
    return Status::OK();
  }
  ...
  if (UNLIKELY(!FLAGS_workload_mgmt_drop_tables.empty())) {
    ...
    for (auto& iter : tables_to_drop) {
      ...
      Status stat = _dropTable(thrift_iface_.get(), WM_DB, iter);
      if (stat.ok()) {
  ...
  RETURN_IF_ERROR(_setupDb(thrift_iface_.get(), WM_DB));

  // Create and/or update the query log table if needed.
  // Fully qualified table name based on startup flags.
  RETURN_IF_ERROR(_tableSchemaManagement(thrift_iface_.get(), QueryLogTableName(false),
      target_schema_version, false));

  // Create and/or update the query live table if needed.
  // Fully qualified table name based on startup flags.;
  RETURN_IF_ERROR(_tableSchemaManagement(thrift_iface_.get(), QueryLiveTableName(false),
