...
# Name of the environment variable that forces the tests for admission controller as a
# separate daemon to run.  Set this variable to 'true' to run the additional tests.
...
class TestAdmissionController(TestAdmissionControllerBase, HS2TestSuite):
  ...
  def __run_assert_systables_query(self, vector, expected_coords=None,
    ...
    result = self.execute_query_using_client(client, query, vector, True)
  ...
  def test_coord_only_pool_add_coord(self, vector):
    ...
    self._start_impala_cluster(
        ...
        workload_mgmt=True)
    self.cluster.impalads[0].service.wait_for_num_known_live_backends(cluster_size + 1)
...
class TestAdmissionControllerWithACService(TestAdmissionController):
  ...
  def setup_method(self, method):
    if not (self.exploration_strategy() == 'exhaustive'
        or os.getenv(RUN_ADMISSIOND_VAR, 'false').strip().lower() == 'true'):
      pytest.skip("runs only in exhaustive or if '{}' environment variable is set to "
