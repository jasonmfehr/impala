...
void _assertCoords(ClusterMembershipMgr& cmm,
    const std::vector<const char*> expected_hostnames) {
  ...
  for (auto expected : expected_hostnames) {
    ...
    EXPECT_TRUE(found) << "did not find expected coordinator '"
        << expected << "' in actual coordinators: " << actual_hostnames.str();
  }
}
