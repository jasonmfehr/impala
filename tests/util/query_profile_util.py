...
def parse_num_rows_fetched_from_cache(profile_text):
  """Parses the number of rows fetched from cache from the query profile text."""
  num_rows_fetched_from_cache = re.search(
      r'\n\s+\-\sNumRowsFetchedFromCache:\s+(\d+)', profile_text)
  assert num_rows_fetched_from_cache is not None, "Number of Rows Fetched From Cache " \
      "not found in query profile"

  return int(num_rows_fetched_from_cache.group(1))
