...
def assert_time_str(expected_str, actual_time_ns, msg, tolerance=1):
  ...
  expected_min = total_nanoseconds - tolerance
  expected_max = total_nanoseconds + tolerance
  assert expected_min <= actual_time_ns <= expected_max, \
      "{0} -- expected: {1}, actual {2}, calculated {3}" \
      .format(msg, expected_str, actual_time_ns, total_nanoseconds)
