...
//   std::mutex mu;
//   std::shared_ptr<bool> wakeup_guard;
//   Ticker<std::chrono::seconds, bool> ticker = Ticker<std::chrono::seconds, bool>(
//       std::chrono::seconds(30), cv, mu, wakeup_guard, true);
//
//   ABORT_IF_ERROR(ticker->Start());
//
//   while(true) {
//     unique_lock<mutex> l(mu);
//     cv.wait(l, [wakeup_guard]{ return *wakeup_guard; });
//     *wakeup_guard = false;
//
//     run_my_code();
//   }
...
class Ticker {
  public:
    ...
    Status Start(const std::string& category, const std::string& name) {
      return Thread::Create(category, name, boost::bind<void>(&Ticker::run, this),
          &my_thread_);
    }
...
// wakeup indicator. The wakeup_value is `true`.
class TickerSB : public Ticker<std::chrono::seconds, bool> {
  public:
    TickerSB(std::chrono::seconds interval, std::condition_variable& cv,
      std::mutex& lock, std::shared_ptr<bool> indicator) : Ticker(interval, cv, lock,
      indicator, true) {}
}; // class TickerSB
