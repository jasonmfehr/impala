...
void SpanManager::StartRootSpan() {
  ...
  root_ = make_unique<TimedSpan>(tracer_, query_id_, "QueryStartTime", "Runtime",
      OtelAttributesMap{
...
void SpanManager::EndRootSpan() {
  ...
  if (LIKELY(root_)) {
    DCHECK(!current_child_) << "Cannot end root span while a child span is active.";
...
unique_ptr<TimedSpan> SpanManager::ChildSpanBuilder(const string& span_name,
  ...
  return make_unique<TimedSpan>(tracer_, full_span_name, "BeginTime", "ElapsedTime",
