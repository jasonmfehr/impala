...
void QueryStateRecord::Init(const ClientRequestState& query_handle) {
  ...
  query_options = query_handle.query_options();
...
bool QueryStateRecordLessThan::operator() (
...
QueryStateExpanded::QueryStateExpanded(const ClientRequestState& exec_state,
  ...
  if (exec_state.schedule() != nullptr) {
    ...
    for (int i =0; i < exec_state.schedule()->backend_exec_params_size(); i++) {
      ...
      PerHostState state;
      state.fragment_instances = b.instance_params_size();
  ...
  if (coord != nullptr) {
   ...
   // Lambda function to recursively walk through a profile.
    std::function<void(RuntimeProfileBase*)> process_exec_profile = [
      ...
      if (auto cntr = profile->GetCounter("ScratchBytesWritten"); cntr != NULL) {
      ...
      if (prof_stack.size() >= 3) {
        if (iequals(prof_stack[1]->name().substr(0, 8), "instance") &&
          ...
          if (auto cntr = profile->GetCounter("ScannerIoWaitTime"); cntr != NULL) {
            scanner_io_wait.emplace(StrCat(
                prof_stack[1]->name(), "::", prof_stack[2]->name()), cntr->value());
          }

          if (auto cntr = profile->GetCounter("DataCacheHitBytes"); cntr != NULL) {
      ...
      // Total Bytes Read
      if (auto cntr = profile->GetCounter("TotalBytesRead"); cntr != NULL) {
    ...
    for (auto b : bytes_read_cache) {
      bytes_read_cache_total += b.second;
    }

    // Per-Node Peak Memory Usage
    for (auto be : coord->BackendResourceUtilization()) {
      TNetworkAddress addr = FromNetworkAddressPB(be.first);
      if(auto host = per_host_state.find(addr); LIKELY(host != per_host_state.end())) {
