...
class TestWorkloadManagementInit(TestWorkloadManagementInitBase):
  ...
  def test_start_3_node_cluster(self):
  ...
  def test_start_1_node_cluster_add_another_coordinator(self):
  ...
  @CustomClusterTestSuite.with_args(
      ...
      catalogd_args="--enable_workload_mgmt", cluster_size=3,
      num_exclusive_coordinators=3)
  def test_start_3_node_cluster_add_another_coordinator(self):
  ...
  @CustomClusterTestSuite.with_args(
      impalad_args="--enable_workload_mgmt --cluster_id=test_wm_init_10 -v=2",
      catalogd_args="--enable_workload_mgmt", cluster_size=1)
  def test_invalid_schema_version(self):
    try:
      self._start_impala_cluster(cluster_size=2, expected_num_impalads=2,
          options=['--impalad_args=--enable_workload_mgmt --cluster_id=test_wm_init_10 '
          '-v=2 --workload_mgmt_schema_version=0.9.9',
          '--catalogd_args=--enable_workload_mgmt'])
    except Exception as e:
      assert "start-impala-cluster.py" in str(e)
      assert str(e).endswith("returned non-zero exit status 1")

    self.assert_impalad_log_contains("ERROR", r"Workload management schema version "
        r"'0.9.9' does not match any valid version. Impalad exiting.")


class TestWorkloadManagementTableSchema(TestWorkloadManagementInitBase):
  """Tests that asserts the workload management db table schema management."""
  ...
  @CustomClusterTestSuite.with_args(
      ...
      catalogd_args="--enable_workload_mgmt", cluster_size=1)
  def test_version_1_tables_not_exist(self):
  ...
  @CustomClusterTestSuite.with_args(
      ...
      catalogd_args="--enable_workload_mgmt", cluster_size=1)
  def test_version_1_tables_exist(self):
  ...
  def test_start_cluster_slow_statestore(self):
    ...
    try:
      ...
      self.wait_for_table_to_appear(self.WM_DB, self.QUERY_TBL_LIVE_NAME, 90, client)
      ...
      # Wait for queued queries to write out to the completed queries table
      self.cluster.get_first_impalad().service.wait_for_metric_value(
        metric_name="impala-server.completed-queries.written", expected_value=1,
        timeout=60, allow_greater=True)
