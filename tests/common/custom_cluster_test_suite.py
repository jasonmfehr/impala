...
class CustomClusterTestSuite(ImpalaTestSuite):
  ...
  def query_id_from_ui(self, section, match_func=None, match_query=None, coord_idx=0):
    """
      ...
      Parameters:
        section:     UI section where the query is located, valie values are
        ...
        service:     Impala coordinator who's UI will be checked. If None, uses the first
                     impalad in the cluster.
  ...
  def query_profile_from_ui(self, query_id, coord_idx=0):
    """
      Wrapper funtion around ImpaladService.read_query_profile_page() to fetch the query
