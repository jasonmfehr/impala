...
/// Equivalent to: CREATE [EXTERNAL] TABLE IF NOT EXISTS {{table_name}} ...
...
/// Equivalent to: ALTER TABLE {{table_name}} ADD IF NOT EXISTS COLUMNS (...).
...
static Status _getTableSchemaVersion(CatalogServiceIf* svc, const string db,
  ...
  if (schema_version_new) {
    *table_version = move(*schema_version_new);
    return Status::OK();
  } else if (schema_version_old) {
