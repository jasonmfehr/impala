...
from tests.util.query_profile_util import parse_db_user, parse_session_id, parse_sql, \
    ...
    parse_num_rows_fetched, parse_num_rows_fetched_from_cache
...
class TestOtelTrace(CustomClusterTestSuite):
  ...
  @CustomClusterTestSuite.with_args(
      impalad_args="-v=2 --cluster_id=test_invalid_sql "
                   ...
                   "--otel_file_pattern={out_dir}/" + TRACE_FILE,
      cluster_size=1, tmp_dir_placeholders=[OUT_DIR], disable_log_buffering=True)
  def test_invalid_sql(self, unique_database, unique_name):
    ...
    query = "insert into {}.{} (id_misnamed, string_col, int_col) values (1, 'a', 10)" \
    ...
    self.__assert_trace(query_id, profile, "test_invalid_sql", 4, "Planning")

  @CustomClusterTestSuite.with_args(
      impalad_args="-v=2 --cluster_id=trace_ddl_createdb --otel_trace_enabled=true "
                   "--otel_trace_exporter=file --otel_file_flush_interval_ms=500 "
                   "--otel_file_pattern={out_dir}/" + TRACE_FILE,
      cluster_size=1, tmp_dir_placeholders=[OUT_DIR], disable_log_buffering=True)
  def test_ddl_createdb(self, unique_name):
    query = "create database {}".format(unique_name)

    try:
      result = self.execute_query_expect_success(self.client, query)

      self.__assert_trace(result.query_id, result.runtime_profile, "trace_ddl_createdb")
    finally:
      self.execute_query_expect_success(self.client, "drop database if exists {}"
          .format(unique_name))

  @CustomClusterTestSuite.with_args(
      impalad_args="-v=2 --cluster_id=trace_ddl_createtbl_success "
                   "--otel_trace_enabled=true --otel_trace_exporter=file "
                   "--otel_file_flush_interval_ms=500 "
                   "--otel_file_pattern={out_dir}/" + TRACE_FILE,
      cluster_size=1, tmp_dir_placeholders=[OUT_DIR], disable_log_buffering=True)
  def test_ddl_createtable_success(self, unique_database, unique_name):
    query = "create table {}.{} (id int, string_col string, int_col int)" \
        .format(unique_database, unique_name)
    result = self.execute_query_expect_success(self.client, query)

    self.__assert_trace(result.query_id, result.runtime_profile,
        "trace_ddl_createtbl_success", 3)

  @CustomClusterTestSuite.with_args(
      impalad_args="-v=2 --cluster_id=trace_ddl_createtbl_fail "
                   "--otel_trace_enabled=true --otel_trace_exporter=file "
                   "--otel_file_flush_interval_ms=500 "
                   "--otel_file_pattern={out_dir}/" + TRACE_FILE,
      cluster_size=1, tmp_dir_placeholders=[OUT_DIR], disable_log_buffering=True)
  def test_ddl_createtable_fail(self, unique_name):
    try:
      ...
      handle = self.execute_query_async(query,
          query_options={"debug_action": "CRS_DELAY_BEFORE_CATALOG_OP_EXEC:SLEEP@500"})
      self.client.wait_for_any_impala_state(handle, [RUNNING], 60)

      # Drop the database after planning to cause the create table to fail.
      self.execute_query_expect_success(self.client, "drop database {} cascade"
          .format(unique_name))
      self.client.wait_for_any_impala_state(handle, [ERROR], 60)

      # Parse common values needed in multiple asserts.
      profile_str = self.client.get_runtime_profile(handle)
      query_id = self.client.get_query_id(handle)
      self.close_query(handle)
    finally:
      self.execute_query_expect_success(self.client, "drop database if exists {} cascade"
          .format(unique_name))

    self.__assert_trace(query_id, profile_str, "trace_ddl_createtbl_fail", 4,
        "QueryExecution")

  @CustomClusterTestSuite.with_args(
      impalad_args="-v=2 --cluster_id=test_dml_success "
                   "--otel_trace_enabled=true --otel_trace_exporter=file "
                   "--otel_file_flush_interval_ms=500 "
                   "--otel_file_pattern={out_dir}/" + TRACE_FILE,
      cluster_size=1, tmp_dir_placeholders=[OUT_DIR], disable_log_buffering=True)
  def test_dml_success(self, unique_database, unique_name):
    query = "create table {}.{} (id int, string_col string, int_col int)" \
        .format(unique_database, unique_name)
  ...
  @CustomClusterTestSuite.with_args(
      impalad_args="-v=2 --cluster_id=test_dml_fail "
                   ...
                   "--otel_file_pattern={out_dir}/" + TRACE_FILE,
      cluster_size=1, tmp_dir_placeholders=[OUT_DIR], disable_log_buffering=True)
  def test_dml_fail(self, unique_database, unique_name):
    ...
    self.execute_query_expect_success(self.client, "alter table {} drop column "
    ...
    self.__assert_trace(query_id, profile, "test_dml_fail", 4, "QueryExecution")

  @CustomClusterTestSuite.with_args(
      impalad_args="-v=2 --cluster_id=test_retry_select "
                   "--otel_trace_enabled=true --otel_trace_exporter=file "
                   "--otel_file_flush_interval_ms=500 "
                   "--otel_file_pattern={out_dir}/" + TRACE_FILE,
  ...
  def __assert_trace(self, query_id, query_profile, cluster_id, trace_cnt=1, err_span="",
      expected_span_count=6, missing_spans=[]):
    # Parse common values needed in multiple asserts.
    ...
    wait_for_file_line_count(trace_file_path, trace_cnt, 5, 1, 2)
  ...
  def __assert_admissioncontrol_attrs(self, spans, root_span_id, query_id, request_pool,
      err_msg="", status="PENDING", queued=False, queued_reason="Admitted immediately"):
    """
      Helper function that asserts the common and span-specific attributes in the
    ...
    self.__assert_attr(adm_ctrl_span.name, adm_ctrl_span.attributes, "Queued",
        queued, "boolValue")
    self.__assert_attr(adm_ctrl_span.name, adm_ctrl_span.attributes, "QueuedReason",
        queued_reason)
  ...
  def __assert_query_exec_attrs(self, spans, query_profile, root_span_id, query_id,
    ...
    self.__assert_scopespan_common(query_exec_span, query_id, False, "QueryExecution", 4,
        status, root_span_id, err_msg)
    ...
    self.__assert_attr(query_exec_span.name, query_exec_span.attributes,
        "NumRowsFetchedFromCache", parse_num_rows_fetched_from_cache(query_profile),
        "intValue")
