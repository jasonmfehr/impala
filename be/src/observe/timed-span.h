...
typedef std::unordered_map<std::string, opentelemetry::common::AttributeValue>
    OtelAttributesMap;
...
public:
  ...
  void SetAttribute(std::string key, const opentelemetry::common::AttributeValue& value)
      noexcept;

  // Adds an event with the given name to the underlying span.
  void AddEvent(std::string name, OtelAttributesMap additional_attributes = {}) noexcept;
