...
class CustomClusterTestSuite(ImpalaTestSuite):
  ...
  def wait_for_wm_init_complete(self, timeout_s=120):
    ...
    # Produce a list of possible catalog versions since multiple updates could be logged
    # as a single new catalog version and thus the version in ret.group(1) might not be
    # the version that gets logged in the catalog topic update applied log message.
    allowed_ver = r'(?:{}'.format(ret.group(1))
    for i in range(int(ret.group(1)) + 1, int(ret.group(1)) + 10):
      allowed_ver += "|{}".format(i)
    allowed_ver += ")"

    self.assert_impalad_log_contains("INFO", r'Catalog topic update applied with '
        r'version: {}'.format(allowed_ver), timeout_s=30, expected_count=-1)
