...
from tests.common.impala_cluster import (
    DEFAULT_BEESWAX_PORT, DEFAULT_HS2_PORT,
    DEFAULT_STATE_STORE_SUBSCRIBER_PORT, DEFAULT_CATALOGD_STATE_STORE_SUBSCRIBER_PORT,
    DEFAULT_CATALOGD1_STATE_STORE_SUBSCRIBER_PORT, DEFAULT_KRPC_PORT,
    ...
    DEFAULT_CATALOG1_SERVICE_PORT, DEFAULT_HS2_HTTP_PORT, DEFAULT_ADMISSIOND_KRPC_PORT,
)
from tests.common.krpc import krpc_tls_validate
...
# kRPC ports to assert.
KRPC_PORTS = [
  DEFAULT_KRPC_PORT,
  DEFAULT_ADMISSIOND_KRPC_PORT,
]
...
class BaseTestServerTls(CustomClusterTestSuite):
  """
     ...
     Checks webserver, Thrift server, and kRPC server ports.
  ...
  def _ciphersuites_allowed(self, vector):
    return sorted(ciph for ciph in self._get_dimension_data(vector)["test_ciphersuites"]
        if self._ciphersuite_filter()(ciph))
  ...
  @pytest.mark.execute_serially
  @pytest.mark.skipif(SKIP_SSL_MSG != "", reason=SKIP_SSL_MSG)
  def test_krpc_ciphersuites_allowed(self, vector):
    """
       Asserts the cluster accepts TLS connections with the configured ciphersuites
       on kRPC ports.
    """
    # Loop through all ports attempting a TLS handshake with each TLS ciphersuite.
    for port in KRPC_PORTS:
      for cipher in self._ciphersuites_allowed(vector):
        assert krpc_tls_validate("localhost", port,
            self._get_tls_ver_str(vector), cipher), \
            "Expected kRPC TLS {} handshake to succeed on 'localhost:{}' with cipher " \
            "'{}'".format(self._get_tls_ver_str(vector), port, cipher)
  ...
  @pytest.mark.execute_serially
  @pytest.mark.skipif(SKIP_SSL_MSG != "", reason=SKIP_SSL_MSG)
  def test_krpc_ciphersuites_notallowed(self, vector):
    """
       Asserts the cluster denies TLS connections with ciphersuites that are not
       configured on kRPC ports.
    """

    # TODO: IMPALA-14907: Remove when tests.common.krpc.krpc_tls_validate supports TLS 1.3
    #                     ciphersuite selection.
    if not self._get_dimension_data(vector)["test_krpc"]:
      pytest.skip("kRPC TLS validation is disabled for TLS version {}"
          .format(self._get_tls_ver_str(vector)))

    # Loop through all ports attempting a TLS handshake with each TLS ciphersuite.
    for port in KRPC_PORTS:
      # Assert a successful TLS handshake for all TLS ciphersuites configured on the
      # cluster.
      for cipher in self._ciphersuites_notallowed(vector):
        assert not \
            krpc_tls_validate("localhost", port, self._get_tls_ver_str(vector), cipher), \
            "Unexpected kRPC TLS {} handshake success on 'localhost:{}', cipher '{}'" \
            .format(self._get_tls_ver_str(vector), port, cipher)
...
class TestServerTlsRSA(BaseTestServerTls):
  """
     ...
     Thrift server, and kRPC TLS tests.
  """
  ...
  def _ciphersuite_filter(cls):
    ...
    """
    return lambda ciph: "ECDSA" not in ciph
...
class TestServerTlsECDSA(BaseTestServerTls):
  ...
  def _ciphersuite_filter(cls):
    ...
    """
    return lambda ciph: "ECDSA" in ciph
