...
void SpanManager::StartChildSpanClose(const Status* cause) {
  ...
  // In a query cancellation scenario, the query could be cancelled before the
  // TraceQuery() has been called. In that case, the Close span will be started during
  // query finalization and before the root span is started.
  if (!started_) {
    DoTraceQuery(false);
    return;
  }
