...
// OtlpFileExporter instance implements the SpanExporter interface. The function parameter
// `exporter` is an in-out parameter that will be populated with the created
// OtlpHttpExporter instance. Returns Status::OK() on success, or an error Status if
// configuration fails.
//
// The file exporter is for test use only.
static Status init_exporter_file(unique_ptr<SpanExporter>& exporter) {
  ...
  exporter = OtlpFileExporterFactory::Create(exporter_opts);
...
Status init_otel_tracer() {
  ...
  VLOG(2) << "OpenTelemetry namespace: " << OPENTELEMETRY_STRINGIFY(OPENTELEMETRY_NAMESPACE);
  ...
  if(FLAGS_otel_trace_exporter == OTEL_EXPORTER_OTLP_HTTP) {
    ...
    RETURN_IF_ERROR(init_exporter_file(exporter));
  }
...
shared_ptr<SpanManager> build_span_manager(const ClientRequestState* crs) {
  ...
  return make_shared<SpanManager>(
      provider_->GetTracer("org.apache.impala.impalad", GetDaemonBuildVersion()), crs);
