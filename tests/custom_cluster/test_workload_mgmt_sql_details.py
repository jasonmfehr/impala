...
class TestWorkloadManagementSQLDetails(TestQueryLogTableBase):
  """Tests that ensure the workload management data that describes the details of the sql
  ...
  def _run_test(self, vector, tpcds_query_num, expected_tables_queried, expected_select,
    ...
    # Execute the query and assert it succeeded.
    client = self.get_client(vector.get_value("protocol"))
    client.execute("use tpcds")
    res = client.execute(query_text)
    assert res.success

    # Wait for the query data to be written to the completed queries table.
  ...
  def test_tpcds_64(self, vector):
    """Asserts the values inserted into the workload management table columns match
       ...
       Query 64 tests addtional subqueries functionality."""
  ...
  def test_sql_having(self, vector):
    ...
    client.execute("use tpcds")
    res = client.execute("select ss_item_sk as Item, count(ss_item_sk) as "
    ...
    # Aggregate Columns
    assert_csv_col(client, self.QUERY_TBL, TQueryTableColumn.AGGREGATE_COLUMNS,
        res.query_id, ["store_sales.ss_item_sk", "store_sales.ss_quantity"])

    # OrderBy Columns
    assert_csv_col(client, self.QUERY_TBL, TQueryTableColumn.ORDERBY_COLUMNS,
        res.query_id, ["store_sales.ss_quantity"])
