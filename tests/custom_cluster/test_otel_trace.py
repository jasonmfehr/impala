...
class TestOtelTrace(CustomClusterTestSuite):
  ...
  def assert_trace(self, query_id, query_profile, cluster_id, trace_cnt=1, err_span="",
      missing_spans=[], async_close=False):
...
class TestOtelTraceSelectsDMLs(TestOtelTrace):
  ...
  def setup_method(self, method):
    ...
    self.client_opts_before = self.client.get_configuration()

  def teardown_method(self, method):
    super(TestOtelTraceSelectsDMLs, self).teardown_method(method)
    self.client.set_configuration(self.client_opts_before)
  ...
  def test_dml_update(self, unique_database, rand_name):
    self.execute_query_expect_success(self.client, "CREATE TABLE {}.{} (id int, "
        ...
        .format(unique_database, rand_name))

    self.execute_query_expect_success(self.client, "INSERT INTO {}.{} (id, string_col) "
        "VALUES (1, 'foo'), (2, 'bar')".format(unique_database, rand_name))

    result = self.execute_query_expect_success(self.client,
        "UPDATE {}.{} SET string_col='a' WHERE id=1".format(unique_database, rand_name))
  ...
  def test_dml_delete(self, unique_database, rand_name):
    self.execute_query_expect_success(self.client, "CREATE TABLE {}.{} (id int, "
        "string_col string) STORED AS ICEBERG TBLPROPERTIES('format-version'='2')"
        .format(unique_database, rand_name))
    ...
    result = self.execute_query_expect_success(self.client,
        "DELETE FROM {}.{} WHERE id=1".format(unique_database, rand_name))
  ...
  def test_dml_delete_join(self, unique_database, rand_name):
    tbl1 = "{}.{}_1".format(unique_database, rand_name)
    ...
    tbl2 = "{}.{}_2".format(unique_database, rand_name)
    self.execute_query_expect_success(self.client, "CREATE TABLE {} STORED AS ICEBERG "
      "TBLPROPERTIES('format-version'='2') AS SELECT id, bool_col, int_col, year, month "
  ...
  def test_ignored_queries(self, unique_database, unique_name):
    ...
    # Syntactic sugar so each query execution can be on a single line.
    execq = self.execute_query_expect_success

    tbl = "{}.{}".format(unique_database, unique_name)
    res_create = execq(self.client, "create table {} (a int)".format(tbl))

    # These queries are not expected to have traces created for them.
    execq(self.client, "COMMENT ON DATABASE {} IS 'test'".format(unique_database))
    execq(self.client, "DESCRIBE {}".format(tbl))
    execq(self.client, "EXPLAIN SELECT * FROM {}".format(tbl))
    execq(self.client, "REFRESH FUNCTIONS functional")
    execq(self.client, "SET ALL")
    execq(self.client, "SHOW TABLES IN {}".format(unique_database))
    execq(self.client, "SHOW DATABASES")
    execq(self.client, "TRUNCATE TABLE {}".format(tbl))
    execq(self.client, "USE functional")
    execq(self.client, "VALUES (1, 2, 3)")

    try:
      execq(self.client, "REFRESH AUTHORIZATION")
    ...
    self.assert_trace(query_id=res_create.query_id,
        query_profile=res_create.runtime_profile, cluster_id="select_dml", trace_cnt=4,
        missing_spans=["AdmissionControl"])
    self.assert_trace(query_id=res_drop.query_id, query_profile=res_drop.runtime_profile,
        cluster_id="select_dml", trace_cnt=4, missing_spans=["AdmissionControl"])
...
class TestOtelTraceDDLs(TestOtelTrace):
  ...
  def test_ddl_create_alter_table(self, vector, unique_database, unique_name):
    ...
    self.assert_trace(query_id=create_result.query_id,
        query_profile=create_result.runtime_profile, cluster_id="trace_ddl", trace_cnt=5,
        missing_spans=["AdmissionControl"])

    self.assert_trace(query_id=alter_success_result.query_id,
        query_profile=alter_success_result.runtime_profile, cluster_id="trace_ddl",
        trace_cnt=5, missing_spans=["AdmissionControl"])

    self.assert_trace(query_id=fail_query_id, query_profile=fail_profile,
        cluster_id="trace_ddl", trace_cnt=5,
        missing_spans=["AdmissionControl", "QueryExecution"], err_span="Planning")
  ...
  def test_compute_stats(self, vector):
    ...
    query_id_cnt, profile_cnt = self.query_id_from_ui(section="completed_queries",
        match_query="SELECT COUNT(*), `year`, `month` FROM functional.alltypes GROUP BY "
                    "`year`, `month`")
    ...
    self.assert_trace(query_id=compute_result.query_id,
        query_profile=compute_result.runtime_profile, cluster_id="trace_ddl",
        trace_cnt=3, missing_spans=["AdmissionControl"])

    self.assert_trace(query_id=query_id_cnt, query_profile=profile_cnt,
        cluster_id="trace_ddl", trace_cnt=3)

    self.assert_trace(query_id=query_id_ndv, query_profile=profile_ndv,
        cluster_id="trace_ddl", trace_cnt=3)

  def test_compute_incremental_stats(self, vector):
    result = self.execute_query_expect_success(self.client,
        "COMPUTE INCREMENTAL STATS functional.alltypes PARTITION (month=1)",
        {"ENABLE_ASYNC_DDL_EXECUTION": vector.get_value('async_ddl')})

    self.assert_trace(query_id=result.query_id, query_profile=result.runtime_profile,
        cluster_id="trace_ddl", trace_cnt=1, missing_spans=["AdmissionControl"])

  def test_refresh(self, vector):
    result = self.execute_query_expect_success(self.client, "REFRESH functional.alltypes",
        {"ENABLE_ASYNC_DDL_EXECUTION": vector.get_value('async_ddl')})
