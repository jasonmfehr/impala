...
static Status _upgradeTable(CatalogServiceIf* svc, const string& ip_addr,
  ...
  t_add_cols.__set_columns(_buildCols(
      [&current_ver, &target_ver](const FieldDefinition& f){
...
static Status _getTableSchemaVersion(CatalogServiceIf* svc, const string& ip_addr,
    ...
    *table_version = move(v);
    ...
    if (!ParseVersion(schema_ver_value.value(), &v).ok()) {
      return Status(StrCat("could not parse version string '", schema_ver_value.value(),
          "' found on the '", WM_SCHEMA_VER_PROP_NAME_1_0_0, "' property of table '",
          db, ".", tbl, "'"));
    }
    *table_version = move(v);
    return Status::OK();
  ...
  return Status(StrCat("Neither property '", WM_SCHEMA_VER_PROP_NAME_1_0_0, "' nor "
      "property '", WM_SCHEMA_VER_PROP_NAME, "' exists on table '", db, ".", tbl, "'" ));
...
static Status _tableSchemaManagement(CatalogServiceIf* svc, const string& ip_addr,
  ...
  RETURN_IF_ERROR(_getTableSchemaVersion(svc, ip_addr, WM_DB, table_name,
  ...
  if (parsed_actual_schema_version == target_schema_version) {
    ...
    RETURN_IF_ERROR(_createTable(svc, ip_addr, table_name, target_schema_version,
        is_system_table));
    ...
    RETURN_IF_ERROR(_upgradeTable(svc, ip_addr, table_name, parsed_actual_schema_version,
